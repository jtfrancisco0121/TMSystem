...
export default function CollectionReceipt(){
  const emptyHeader = { date: new Date().toISOString().slice(0,10), receivedFrom:'', address:'', orNumber:'' }
  const { header, setHeader, items, updateItem, totals, saveRecord, preview, setPreview, openPreview, clear } = useFinancials<typeof emptyHeader>(emptyHeader, 'savedReceipts')

  function save(){ saveRecord(); alert('Saved receipt to local state') }
  function printNow(){ window.print() }
  function clearForm(){ if(!confirm('Clear receipt form?')) return; clear() }
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={save} className="px-3 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700">Save Receipt</button>
          <button onClick={openPreview} className="px-3 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Preview</button>
          <button onClick={printNow} className="px-3 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700">Print</button>
      ...
      <div className="bg-white shadow rounded-lg p-4 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm text-gray-600">Received From</label>
            <input className="mt-1 w-full px-3 py-2 border rounded-md" value={header.receivedFrom} onChange={e=>setHeader(h=>({...h, receivedFrom:e.target.value}))} />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm text-gray-600">OR No.</label>
            <input className="mt-1 w-full px-3 py-2 border rounded-md" value={header.orNumber} onChange={e=>setHeader(h=>({...h, orNumber:e.target.value}))} />
          </div>
        </div>
      </div>
      ...
      {preview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white w-[90%] max-w-3xl rounded-lg shadow-lg p-6 overflow-auto max-h-[90%] printable-receipt">
            <div className="flex justify-between items-start">
              <h3 className="text-lg font-semibold">Collection Receipt Preview</h3>
              <button onClick={()=>setPreview(null)} className="text-gray-500 hover:text-gray-700">Close</button>
            </div>
            <div className="mt-4">
              <div className="flex justify-between">
                <div>
                  <div className="font-semibold">{preview.header.receivedFrom}</div>
                  <div className="text-sm text-gray-600">{preview.header.address}</div>
                  <div className="text-sm text-gray-600">OR#: {preview.header.orNumber}</div>
                </div>
